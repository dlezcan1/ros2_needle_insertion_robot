...
class RobotControlWidget(QWidget):
    ...
    def __init__(self, node, parent=None):
        ...
        self.pushButton_zeroX.clicked.connect(lambda: self.pushButton_srv_event("/stage/axis/zero/x"))
        self.pushButton_zeroY.clicked.connect(lambda: self.pushButton_srv_event("/stage/axis/zero/y"))
        self.pushButton_zeroZ.clicked.connect(lambda: self.pushButton_srv_event("/stage/axis/zero/z"))
        self.pushButton_zeroLS.clicked.connect(lambda: self.pushButton_srv_event("/stage/axis/zero/linear_stage"))
        self.pushButton_zeroAll.clicked.connect(self.pushButton_zeroAll_event)


        self._node.create_subscription(Float32, "/stage/axis/position/x", lambda msg: self.axis_position_callback(msg, "x"), 10)
        ...
        self._node.create_subscription(Bool, "/stage/axis/state/on/x", lambda msg: self.axis_state_callback(msg, "x"), 10)
        ...
        # - relative movements
        self.label_relMoveX_min.setText(str(self.horizontalSlider_relMoveX.minimum()))
        self.label_relMoveX_max.setText(str(self.horizontalSlider_relMoveX.maximum()))
        ...
        # - absolute movements
        self.label_absMoveX_min.setText(str(self.horizontalSlider_absMoveX.minimum()))
        self.label_absMoveX_max.setText(str(self.horizontalSlider_absMoveX.maximum()))
        ...
        self.lineedit_relMoveX.setInputMask("####")
        self.lineedit_relMoveY.setInputMask("####")
        self.lineedit_relMoveZ.setInputMask("####")
        self.lineedit_relMoveLS.setInputMask("####")
        
        self.lineedit_absMoveX.setInputMask("####")
        self.lineedit_absMoveY.setInputMask("####")
        self.lineedit_absMoveZ.setInputMask("####")
        self.lineedit_absMoveLS.setInputMask("####")

        self.horizontalSlider_relMoveX.valueChanged.connect(lambda: self.lineedit_move_event(self.horizontalSlider_relMoveX.value(), self.lineedit_relMoveX))
        self.horizontalSlider_relMoveY.valueChanged.connect(lambda: self.lineedit_move_event(self.horizontalSlider_relMoveY.value(), self.lineedit_relMoveY))
        self.horizontalSlider_relMoveZ.valueChanged.connect(lambda: self.lineedit_move_event(self.horizontalSlider_relMoveZ.value(), self.lineedit_relMoveZ))
        self.horizontalSlider_relMoveLS.valueChanged.connect(lambda: self.lineedit_move_event(self.horizontalSlider_relMoveLS.value(), self.lineedit_relMoveLS))

        self.horizontalSlider_absMoveX.valueChanged.connect(lambda: self.lineedit_move_event(self.horizontalSlider_absMoveX.value(), self.lineedit_absMoveX))
        self.horizontalSlider_absMoveY.valueChanged.connect(lambda: self.lineedit_move_event(self.horizontalSlider_absMoveY.value(), self.lineedit_absMoveY))
        self.horizontalSlider_absMoveZ.valueChanged.connect(lambda: self.lineedit_move_event(self.horizontalSlider_absMoveZ.value(), self.lineedit_absMoveZ))
        self.horizontalSlider_absMoveLS.valueChanged.connect(lambda: self.lineedit_move_event(self.horizontalSlider_absMoveLS.value(), self.lineedit_absMoveLS))

        self.pushButton_relMoveX.clicked.connect(lambda: self.pushButton_move_event("x", isAbsolute=False))
        self.pushButton_relMoveY.clicked.connect(lambda: self.pushButton_move_event("y", isAbsolute=False))
        self.pushButton_relMoveZ.clicked.connect(lambda: self.pushButton_move_event("z", isAbsolute=False))
        self.pushButton_relMoveLS.clicked.connect(lambda: self.pushButton_move_event("linear_stage", isAbsolute=False))

        self.pushButton_absMoveX.clicked.connect(lambda: self.pushButton_move_event("x", isAbsolute=True))
        self.pushButton_absMoveY.clicked.connect(lambda: self.pushButton_move_event("y", isAbsolute=True))
        self.pushButton_absMoveZ.clicked.connect(lambda: self.pushButton_move_event("z", isAbsolute=True))
        self.pushButton_absMoveLS.clicked.connect(lambda: self.pushButton_move_event("linear_stage", isAbsolute=True))

        self.pushButton_abort.clicked.connect(lambda: self.pushButton_srv_event("/stage/abort"))

    def axis_position_callback(self, msg, axisName):
        if (axisName == "x"):
            self.lcdNumber_X.display(msg.data)
        if (axisName == "y"):
    ...
    def axis_state_callback(self, msg, axisName):
        if (axisName == "x"):
            self.label_stateX.setText("ON" if msg.data else "OFF")
            self.label_stateX.setStyleSheet("background-color: green; border: 1px solid black;" if msg.data else "background-color: red; border: 1px solid black;")
        if (axisName == "y"):
    ...
    def pushButton_toggleAll_event(self):
        self.pushButton_srv_event("/stage/axis/state/toggle/x")
    ...
    def pushButton_zeroAll_event(self):
        self.pushButton_srv_event("/stage/axis/zero/x")
    ...
    def pushButton_move_event(self, axisName, isAbsolute=False):
        ...
        if (axisName == "x"):
            if (isAbsolute):
                slider   = self.horizontalSlider_absMoveX
                lineEdit = self.lineedit_absMoveX
            else:
                slider   = self.horizontalSlider_relMoveX
                lineEdit = self.lineedit_relMoveX
        elif (axisName == "y"):
            if (isAbsolute):
